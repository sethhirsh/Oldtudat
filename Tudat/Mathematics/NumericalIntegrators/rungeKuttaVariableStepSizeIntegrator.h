...
#define TUDAT_RUNGEKUTTAVARIABLESTEPSIZEINTEGRATOR_H
...
public:
    ...
    //! Set the relative error tolerance per item in the state vector
    /*!
     * Set the relative error tolerance per item in the state vector
     * \param relativeErrorTolerance The relative error tolerance, for each individual state
     * vector element
        ...
        const IndependentVariableType stepSize )
{
    ...
    for ( int stage = 0; stage < coefficients_.cCoefficients.rows( ); stage++ )
    {
        ...
        stateDerivatives.push_back(
                    stateDerivativeFunction_( currentInterval_ +
...
//! Typedef of a Runge-Kutta variable step size integrator with VectorXds as state and state derivative and double as independent variable
/*!
 * Typedef of a Runge-Kutta variable step size integrator with VectorXds as state and state
...
#endif // TUDAT_RUNGEKUTTAVARIABLESTEPSIZEINTEGRATOR_H
